...
public class Peripheral extends BluetoothGattCallback {
    ...
    public void onServicesDiscovered(BluetoothGatt gatt, int status) {
        ...
        //将发现的服务发送通知告诉RN
        this.SendRnServices(gatt);
        //发送特征值
        this.SendRnCharacteristic(gatt);
    }
...
//		//将发现的特征值发送告诉RN
//		this.SendRnCharacteristic(characteristic);
    }
     ...
     * 自定义服务通知的格式
     */
    public void SendRnServices(BluetoothGatt gatt) {
        WritableArray servicesArray = Arguments.createArray();
        if (connected && gatt != null) {
            for (Iterator<BluetoothGattService> it = gatt.getServices().iterator(); it.hasNext(); ) {
                BluetoothGattService service = it.next();
        ...
        WritableMap map = Arguments.createMap();
        map.putString("peripheralUUID",device.getAddress());
        map.putArray("services",servicesArray);
        sendEvent("BleManagerDiscoverService",map);
    }

    /**
     * 将发现特征值发送给RN
     */
    public void SendRnCharacteristic(BluetoothGatt gatt) {
        for (Iterator<BluetoothGattService> it = gatt.getServices().iterator(); it.hasNext(); ) {
            BluetoothGattService service = it.next();
            ...
            for (Iterator<BluetoothGattCharacteristic> itCharacteristic = service.getCharacteristics().iterator(); itCharacteristic.hasNext(); ) {
                ...
                //发送特征值
                byte[] dataValue = characteristic.getValue();
                WritableMap charactersMap = Arguments.createMap();
                charactersMap.putString("UUID", characteristic.getUuid().toString());
                charactersMap.putArray("value",characteristic.getValue() ==null? null:BleManager.bytesToWritableArray(dataValue));

                WritableArray array = Arguments.createArray();
                array.pushMap(charactersMap);

                WritableMap map = Arguments.createMap();
                map.putString("service", characteristic.getService().getUuid().toString());
                map.putArray("characters",array);
                sendEvent("BleManagerDiscoverCharacteristic", map);
            }
        }
    }
